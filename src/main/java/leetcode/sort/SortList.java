...
public class SortList {
	...
	private ListNode split(ListNode head, int size) {
		while (head != null && size > 1) { // compare size with 1 since head should be the last element of the first part
			head = head.next;
			size--;
		}
	...
	private ListNode merge(ListNode l1, ListNode l2, ListNode head) {
		...
		while (current.next != null) current = current.next; // advance current to the end
