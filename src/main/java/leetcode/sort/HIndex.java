...
public class HIndex {
    ...
    public int hIndex_countingSort(int[] citations) {
        ...
        int n = citations.length;
        int[] papers = new int[n + 1]; // count the number of papers that have
                                       // been cited i times,
    ...
    public int hIndexII(int[] citations) {
        // The basic idea is to search for the first index from the sorted array
        // such that
        // citations[index] >= length(citations) - index
        // And return (length - index) as the result.
        //
        // Just binary search, each time check citations[mid]
        // case 1: citations[mid] == len-mid, then it means there are
        // citations[mid] papers that have at least citations[mid] citations
        // case 2: citations[mid] > len-mid, then it means there are
        // citations[mid] papers that have more than citations[mid] citations,
        ...
        while (lo <= hi) { // use lo <= hi to handle {0, 0}
            ...
            if (citations[mid] > n - mid) { // citation number is bigger, paper
                                            // is not enough, go to left and
                                            // find a smaller citation
                hi = mid - 1;
            } else {
                lo = mid + 1;
            }
