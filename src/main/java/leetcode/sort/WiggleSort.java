...
public class WiggleSort {
    ...
    // previous wiggled elements (nums[i - 2] >= nums[i - 1]),
    // we know after swap the sequence is ensured to be nums[i - 2] > nums[i -
    // 1] < nums[i], which is wiggled.
    // similarly,
    ...
    public void wiggleSortII_sortAndExtraSpace(int[] nums) {
        ...
        // find the median
        // if there are odd number of elements, the left half must be 1 more
        // than the right, not reverse.
        // That is because the last element must be indexed as even, and since
        // nums[even] < nums[odd],
        // so the last number must be in the smaller half.
        // start from the end of both halves to handle equal elements. e.g
        // [4,5,5,6], make sure equal elements are not neighbors
    ...
    public void wiggleSortII_quickSelectAndExtraSpace(int[] nums) {
        ...
        // three-way partition, ensure elements equal to median are in the
        // middle
