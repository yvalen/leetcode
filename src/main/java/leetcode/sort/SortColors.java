...
public class SortColors {
	...
	public void sortColors_withThreePointers(int[] nums) {
		...
		while (mid <= hi) {
			if (nums[mid] == 0) {
				// exch(nums, lo, mid);  // no need to do swap because we know the value to put at lo
				nums[mid] = nums[lo];
				nums[lo] = 0;
				mid++;
				lo++;
				...
				//exch(nums, mid, hi);
				//mid++; // cannot increment mid here because we need to process the element that has just been swapped here
				nums[mid] = nums[hi];
				nums[hi] = 2;
				hi--;
			}
