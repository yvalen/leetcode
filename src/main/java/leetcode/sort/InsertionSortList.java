...
public class InsertionSortList {
	// starts with an initially empty (and therefore trivially sorted) list. The input items are taken off the list one at a time, 
	// and then inserted in the proper place in the sorted list. When the input list is empty, the sorted list has the desired result.
	...
	public ListNode insertionSortList_withDummyHead(ListNode head) {
		...
		while (current != null) {
			...
			while(prev.next != null && prev.next.val < current.val) { // use prev.next to find the position
				prev = prev.next;
			}
