...
public class NextGreaterElement {
	 ...
	 * for nums1's elements in the corresponding places of nums2. The Next Greater Number of a number x in nums1 is the first greater number to its 
	...
	public int[] nextGreaterElement(int[] findNums, int[] nums) {
        Map<Integer, Integer> greaterElemMap = new HashMap<>();  // stores the next greater elem for every elem in nums if exists
        Stack<Integer> stack = new Stack<>(); // stores a decreasing sub-sequence in nums
        for (int num : nums) {
        	// whenever we see a number x greater than stack.peek() we pop all elements 
        	// less than x and for all the popped ones, their next greater element is x
	...
	public int[] nextGreaterElementsII(int[] nums) {
		...
		// start from end of the array look for the first element (index) in the stack which is greater than the current one
