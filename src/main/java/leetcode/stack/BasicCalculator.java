...
public class BasicCalculator {
    /*
     * LEETCODE 224 Implement a basic calculator to evaluate a simple expression
     * string. The expression string may contain open ( and closing parentheses
     * ), the plus + or minus sign -, non-negative integers and empty spaces .
     * You may assume that the given expression is always valid. Some examples:
     * "1 + 1" = 2 " 2-1 + 2 " = 3 "(1+(4+5+2)-3)+(6+8)" = 23
     * 
     * Company: Google Difficulty: hard Similar Questions:
     * 150(EvaluateReversePolishNotation), 227(Basic Calculator II),
    ...
    public int calculate(String s) {
        ...
        for (char ch : s.toCharArray()) {
            if (Character.isDigit(ch)) {
                number = number * 10 + (ch - '0'); // need to multiply by 10 to
                ...
                // must be
                // + or - and number has been reset to 0 when processing that
                // char
                result = 0;
                sign = 1;
            } else if (ch == ')') {
                result += sign * number; // value enclosed in parenthesis
                result *= stack.pop(); // apply the sign to the value enclosed
                                       // in parenthesis
                result += stack.pop(); // add the value enclosed in parenthesis
                                       // to previous result
    ...
    /*
     * LEETCODE 227 Implement a basic calculator to evaluate a simple expression
     ...
     * 
     * Company: Airbnb Difficulty: medium Similar Questions:
     * 244(BasicCalculator), 282(ExpressionAddOperators)
     */
    public int calculateII(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (Character.isDigit(ch)) {
                ...
                number = number * 10 + (ch - '0'); // use ch-'0' to get current
                                                   // value
            }
            if (ch == '+' || ch == '-' || ch == '*' || ch == '/' || i == s.length() - 1) { // need
                                                                                           // to
                                                                                           // process
                                                                                           // the
                                                                                           // last
                                                                                           ...
                                                                                           // if
                                                                                           // here
                if (op == '*' || op == '/') { // subtract the top element from
                                              // result before multiply/divide
                    result -= stack.peek();
                }
                switch (op) {
                case '+':
                ...
                op = ch;
                number = 0;
                result += stack.peek(); // update result with the number just
                                        // pushed onto stack
            }
        }
        return result;
