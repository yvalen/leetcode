 ...
 * spaces, square brackets are well-formed, etc. Furthermore, you may assume that t
 * he original data does not contain any digits and that digits are only for those 
 ...
 * Company: Google
 * Difficulty: medium
 * Similar Questions: 471(EncodeStringWithShortestLength), 726
 */
    ...
    public String decodeString(String s) {
        ...
        for (char c : s.toCharArray()) {
            if (Character.isDigit(c)) {
                ...
                // end of count, push it to count stack and reset the counter
                // for the next count
                ...
                sb = sbStack.pop(); // append the result to the top element on
                                    // the stack
                while (repeat-- > 0) {
                    sb.append(str);
                }
    ...
    private String dfs(String s, int[] index) {
        ...
        while (index[0] < s.length()) {
            if (Character.isDigit(s.charAt(index[0]))) {
                ...
                index[0]++; // need to increment the global index to process the
