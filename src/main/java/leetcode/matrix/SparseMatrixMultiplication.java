...
public class SparseMatrixMultiplication {
	public int[][] multiply(int[][] A, int[][] B) {
        ...
        for (int i = 0; i < m; i++) {
        	for (int j = 0; j < B.length; j++) {
        		if (A[i][j] != 0) { // only perform multiplication when the element is non-zero
        			for (int k = 0; k < n; k++) {
        				if (B[j][k] != 0) {
	...
	// store the sparse matrix in list of lists(LIL), one list per row, with each entry containing the column index and the value.
