...
public class LonelyPixel {
	...
	private boolean isLonelyPixel(char[][] picture, int row, int col) {
		for (int i = 0; i < picture[0].length; i++) {
			if (i != col && picture[row][i] == 'B') return false;
		}
		for (int i = 0; i < picture.length; i++) {
			if (i != row && picture[i][col] == 'B') return false;
		}
		return true;
	}
	...
	public int findLonelyPixel_withArray(char[][] picture) {
        ...
        int m = picture.length, n = picture[0].length;
        int[] rowCount = new int[m], colCount = new int[n]; // stores the count of black pixels for each row and column
        for (int i = 0; i < m; i++) {
        	for (int j = 0; j < n; j++) {
        		if (picture[i][j] == 'B') {
        			rowCount[i]++;
	...
	public int findLonelyPixel(char[][] picture) {
        ...
        for (int i = 0; i < m; i++) {
        	for (int j = 0; j < n; j++) {
        		if (picture[i][j] == 'B') {
        			...
        			if (picture[0][j] < 'Y' || picture[0][j] != 'V') picture[0][j]++; // picture[0][0] stores the state of the first column
        		}
        	}
        }
        ...
        for (int i = 0; i < m; i++) {
        	for (int j = 0; j < n; j++) {
        		if (picture[i][j] < 'W' && (picture[0][j] == 'C' || picture[0][j] == 'X')) { // need to check for < 'W' so that the first row can be handled properly
