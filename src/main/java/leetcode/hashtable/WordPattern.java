...
public class WordPattern {
	 ...
	 * Notes: You may assume pattern contains only lower case letters, and str contains lower case letters separated by a single space. 
	 * 
	...
	public boolean wordPattern(String pattern, String str) {
        ...
        if (pattern.length() != strAry.length) return false;
        ...
        for (int i = 0; i < pattern.length(); i++) {
        	...
        	String s = strAry[i];
        	if (!mapP.getOrDefault(p, -1).equals(mapS.getOrDefault(s, -1))) return false;
        	mapS.put(s,  i);
        	mapP.put(p, i);
        }
        return true;
