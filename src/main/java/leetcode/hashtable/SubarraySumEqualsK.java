...
public class SubarraySumEqualsK {
    ...
    // if the cumulative sum up to two indices, say i and j is at a difference
    // of k i.e. if sum[i]−sum[j]=k
    ...
    public int subarraySum_hashMap(int[] nums, int k) {
        ...
        int count = 0, sum = 0;
        Map<Integer, Integer> map = new HashMap<>(); // map stores the number of
                                                     // times a sum has occurred
    ...
    public int subarraySum_withSumArray(int[] nums, int k) {
        ...
        sums[0] = 0;
        // cumulative sum is calculated by adding the previous sum to current
        // num
        for (int i = 1; i <= n; i++)
