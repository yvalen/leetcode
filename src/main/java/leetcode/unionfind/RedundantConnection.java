...
public class RedundantConnection {
	 ...
	 * was not an edge that already existed. The resulting graph is given as a 2D-array of edges. Each element 
	 * of edges is a pair [u, v] with u < v, that represents an undirected edge connecting nodes u and v.
	 ...
	 * Company: Google
	 * Difficulty: medium
	 * Similar Questions: 721(AccountMerge)
	 */
	public int[] findRedundantConnection_dfs(int[][] edges) {
		...
		for (int [] edge : edges) {
			if (hasCycle(graph, edge[0], edge[1], 0)) return edge;
	...
	private boolean hasCycle(Map<Integer, List<Integer>> graph, int start, int end, int prev) {
		if (graph.get(start).contains(end)) return true; // end has been visited via start already
		for (int neighbor : graph.get(start)) {
			if (neighbor == prev) continue; // skip if this is the node leads to start
			if (hasCycle(graph, neighbor, end, start)) return true;
