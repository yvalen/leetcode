 ...
 * chosen from 1 to N, and was not an edge that already existed. The resulting graph is given 
 * as a 2D-array of edges. Each element of edges is a pair [u, v] with u < v, that represents 
 ...
 * Company: Google 
 * Difficulty: medium 
 * Similar Questions: 721(AccountMerge), 685(Redundant Connection II)
 */
public class RedundantConnection {
    // Time Complexity: O(N^2) where NNN is the number of vertices (and also the number of edges) in the graph. 
    ...
    // Time complexity: O(N), N is the number of vertices (and also the number of edges) 
    //  We make up to N queries of union, which takes (amortized) O(α(N)) ~ O(1) time.
