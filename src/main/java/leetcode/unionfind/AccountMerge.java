...
public class AccountMerge {
	...
	public List<List<String>> accountsMerge_dfs(List<List<String>> accounts) {
		// build the graph, a map that maps email to a list of accounts (use index as account id)
		...
		// dfs on each account, find accounts that are linked to this account via common emails
		boolean[] visited = new boolean[accounts.size()];
		List<List<String>> result = new ArrayList<>();
		for (int i = 0; i < accounts.size(); i++) {
			Set<String> accountEmails = new TreeSet<>(); // collects all emails that are linked to this account
			dfs(accounts, emailAccountMap, i, visited, accountEmails);
			if (!accountEmails.isEmpty()) { // need to check for empty here, otherwise linked account will appear in the result wih empty email list
	...
	private void dfs(List<List<String>> accounts, Map<String, List<Integer>> emailAccountMap, int account, boolean[] visited, Set<String> accountEmails) {
	...
	public List<List<String>> accountsMerge(List<List<String>> accounts) {
        ...
        Map<String, Integer> emailAccountMap = new HashMap<>(); // store the email to parent account mapping
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = 1; j < emails.size(); j++) {
				...
				if (emailAccountMap.containsKey(email)) { // if this email belongs to an account already, union the current account with that account
		...
		for (int i = 0; i < n; i++) {
			...
			resultMap.putIfAbsent(parent, new TreeSet<>()); // use tree set for sortimg
			resultMap.get(parent).addAll(accountEmailMap.get(i)); // add all emails associated with current account to parent's email set
		}
