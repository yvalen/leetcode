...
public class AdditiveNumber {
    // Generate the first and second of the sequence, check if the rest of the
    // string match the sum recursively.
    // i and j are length of the first and second number. i should in the range
    // of [0, n/2]. The length of their sum should >= max(i,j)
    ...
    private boolean isValid(int i, int j, String num) {
        ...
        for (int k = i + j; k < num.length(); k += sum.length()) {
            val2 += val1; // compute the sum and save it in val2 for the next
                          // iteration to use
            val1 = val2 - val1; // update val1 with the old value of val2 for
