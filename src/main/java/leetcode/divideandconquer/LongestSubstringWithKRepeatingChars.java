...
public class LongestSubstringWithKRepeatingChars {
    ...
    // because it will always be too infrequent and thus can't be part of any ok
    // substring
    // and make the most out of the splits.
    // Time complexity: With O(1) alphabet size it's O(n) because it does O(n)
    // work at each level
    // (of the call tree) and there are O(1) levels. With unlimited alphabet,
    // each level takes O(n^2)
