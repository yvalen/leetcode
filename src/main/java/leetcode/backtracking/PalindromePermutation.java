...
public class PalindromePermutation {
	public List<String> generatePalindromes(String s) {
		...
		for (Character c : s.toCharArray()) {
			...
			oddCount += countMap.get(c) % 2 != 0 ? 1 : -1; // need to decrement by one for event count to offset the increment in the previous count
		}
		
		// palindrome can only have 1 odd count
		if (oddCount > 1) return Collections.emptyList();
		...
		for (Map.Entry<Character, Integer> entry : countMap.entrySet()) {
			...
			for (int i = 0; i < count / 2; i++) charList.add(entry.getKey());
		}
	...
	private void helper(List<Character> charList, List<String> result, String mid, StringBuilder sb, boolean[] used) {
		if (sb.length() == charList.size()) {
			...
			sb.reverse(); // need to restore the order of sb for backtracking
			return;
		}
		...
		for (int i = 0; i < charList.size(); i++) {
			...
			if (i > 0 && charList.get(i) == charList.get(i-1) && !used[i-1]) continue; // check !used[i-1] is faster than check used[i-1]
