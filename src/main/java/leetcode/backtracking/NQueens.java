...
public class NQueens {
    /*
     * The n-queens puzzle is the problem of placing n queens on an n×n chess
     * board such that no two queens attack each other. Thus a solution requires
     * that no two queens share the same row, column, or diagonal. Given an
     * integer n, return all distinct solutions to the n-queens puzzle. Each
     ...
     * respectively. For example, there exist two distinct solutions to the
     * 4-queens puzzle: [ [".Q..", // Solution 1 "...Q", "Q...", "..Q."],
     * ["..Q.", // Solution 2 "Q...", "...Q", ".Q.."] ]
     */
    ...
    public List<List<String>> solveNQueens_withOneDArray(int n) {
        List<List<String>> result = new ArrayList<>();
        // position array stores the column index where queen is placed on each
        // row
    ...
    private boolean isValid(int[] position, int row) {
        for (int i = 0; i < row; i++) { // check the rows that has been
                                        // processed so far
            if (position[i] == position[row] || // check column
                    Math.abs(position[i] - position[row]) == row - i // check
                                                                     // diagonal,
                                                                     // diagonal
                                                                     // has
                                                                     // row==col
