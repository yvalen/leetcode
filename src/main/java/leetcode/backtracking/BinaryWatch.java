...
public class BinaryWatch {
	...
	public List<String> readBinaryWatch_backtrack(int num) {
		...
		for (int i = 0; i <= num; i++) {
			...
			for (int hour : hours) {
				if (hour >= 12) continue;  // hour is in the range of 0-11
				int len1 = sb.length();
				sb.append(hour).append(":");
				for (int minute : minutes) {
					if (minute >= 60) continue; // minute is in the range of 0-59
					int len2 = sb.length();
					if (minute < 10) {
						sb.append(0);
					}
	...
	//
	// Bit Manipulation
	// go through the possible times and collect those with the correct number of one-bits.
	public List<String> readBinaryWatch_bit(int num) {
		List<String> result = new ArrayList<>();
		for (int h = 0; h < 12; h++) {
			for (int m = 0; m < 60; m++) {
				// hours occupy 4 bits, minutes occupy 6 bits
				// shift hours by 6 bits to differentiate hours and minutes by bit,
				// we could also use: h*64+m, h+m*16, (h<<6)+m
				if ((Integer.bitCount(m) + Integer.bitCount(h)) == num) {
					result.add(String.format("%d:%02d", h, m)); // %02d means an integer, left padded with zeros up to 2 digits.
