...
public class RegularExpressionMatching {
			...
			for (int j = 0; j < p.length(); j++) {
				if (p.charAt(j) == s.charAt(i) || p.charAt(j) == '.') {
					...
					if (p.charAt(j-1) == s.charAt(i) || p.charAt(j-1) == '.') {
						dp[i+1][j+1] = dp[i][j+1] ||  // a* is counted as multiple a
								dp[i+1][j] || // a* is counted as a single a
								dp[i+1][j-1]; // a* is counted as empty
								
					}
					else {
						dp[i+1][j+1] = dp[i+1][j-1]; // * is counted as empty
					}
				}
			}
		}
	...
	public boolean isMatch_recursiveHelper(String s, String p, int sIndex, int pIndex) {
		...
		if (pIndex < p.length() && p.charAt(pIndex+1) != '*') {
			if (sIndex == s.length()) return false;  // all chars in s has been checked
		...
		// * is counted as empty
		if (isMatch_recursiveHelper(s, p, sIndex, pIndex+2)) return true;
		
		// * is counted as 1 or more characters
		for (int i = sIndex; i < s.length(); i++){
			if (!isMatchAt(s, p, i, pIndex)) return false;
			if (isMatch_recursiveHelper(s, p, i+1, pIndex+2)) { // use i+1 to match one or more
				return true;
			}
		}
