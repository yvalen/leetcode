...
public class MaximumRectangle {
    ...
    // maintain a column length of integer array height, scan and update row by
    // row to find out the largest rectangle of each row.
    ...
    public int maximalRectangle_withStack(char[][] matrix) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            for (int j = 0; j <= n; j++) {
                int h = j == n ? 0 : heights[j]; // when all elements are
                                                 // examined set h to 0 to
                ...
                if (stack.isEmpty() || h >= heights[stack.peek()]) {
                    ...
                    j--; // counteract i++ in the for loop so that the current
                         // element will be put onto the stack
                }
            }
        }
        return max;
    ...
    public int maximalRectangle_dp(char[][] matrix) {
        ...
        int[] height = new int[n]; // from top to this position, there are how
                                   // many '1'
        int[] leftBoundary = new int[n]; // at current position, what is the
                                         // index of left bound of the rectangle
                                         // with height[j]. 0 means at this
                                         // position, no rectangle.
        int[] rightBoundary = new int[n]; // means the right bound index of this
                                          // rectangle. 'n' means no rectangle.
