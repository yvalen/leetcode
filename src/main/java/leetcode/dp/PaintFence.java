...
public class PaintFence {
	 ...
	 * - from 2.2, since previous two are not the same, you can either paint the same color this time (diff*1) ways to do so, 
	 * or stick to paint differently (diff*(k-1)) times.
	 * Here you can conclude, when seeing back from the next level, ways to paint the same, or variable same would equal to diff*1 = diff, 
	 * and ways to paint differently, variable diff, would equal to same*(k-1)+diff*(k-1) = (same + diff)*(k-1)
	 */
	public int numWays(int n, int k) {
        ...
        for (int i = 3; i <= n; i++) {
        	...
        	same = temp;  // post at i must be different from post at i-2, hence use diff value of i-1 for painting i in the same color as i-1
        }
