...
public class DistinctSubsequence {
    public int numDistinct(String s, String t) {
        ...
        // dp[i+1][j+1] stores the number of times s(0...j) contains t(0...i) as
        ...
        for (int i = 0; i < t.length(); i++) {
            for (int j = 0; j < s.length(); j++) {
                if (t.charAt(i) == s.charAt(j)) {
                    ...
                    // dp[i+1][j] - don't use the matching character in current
                    // subsequence
                    dp[i + 1][j + 1] = dp[i][j] + dp[i + 1][j];
                } else {
                    // when not equal, we have the same number of distinct
                    // subsequences
