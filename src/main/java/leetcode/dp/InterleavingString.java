...
public class InterleavingString {
    ...
    private boolean bruteForceHelper(String s1, String s2, String s3, int i, int j, String result) {
        ...
        if (i < s1.length()) {
            if (bruteForceHelper(s1, s2, s3, i + 1, j, result + s1.charAt(i)))
                return true;
        }

        if (j < s2.length()) {
            if (bruteForceHelper(s1, s2, s3, i, j + 1, result + s2.charAt(j)))
                return true;
        }
    ...
    public boolean isInterleave_dpTwoD(String s1, String s2, String s3) {
        ...
        // is at ith position and s2 is at jth position
        boolean[][] dp = new boolean[s1.length() + 1][s2.length() + 1];
        dp[0][0] = true; // 0th position means empty string, empty and empty
                         // matches empty

        // s2 is empty, it is interleaving when previous position is
        // interleaving and current position of s1 and s3 matches
        for (int i = 1; i <= len1; i++) {
            dp[i][0] = dp[i - 1][0] && (s1.charAt(i - 1) == s3.charAt(i - 1));
        }

        // s1 is empty, it is interleaving when previous position is
        // interleaving and current position of s2 and s3 matches
    ...
    public boolean isInterleave_dpOneD(String s1, String s2, String s3) {
        ...
        // 1-d array to store the results of the prefixes of the strings
        // processed so far.
