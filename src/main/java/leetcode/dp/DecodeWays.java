...
public class DecodeWays {
	...
	public int numDecodings_bottomUp(String s) {
		...
		int n = s.length();
		int[] dp = new int[n+1]; // dp[i] is the number of ways to decode substring (i, end);
		dp[n] = 1; // only one way when string is empty
		dp[n-1] = s.charAt(n-1) == '0' ? 0 : 1;  // if last char is 0, it cannot be used; otherwise there is one way
		for (int i = n-2; i >= 0; i--) {
			if (s.charAt(i) == '0') continue;  // 0 cannot be used alone, it needs to be used together with the letter before it
	...
	public int numDecodingsII(String s) {
        ...
        long w1 = ways(s.charAt(0));
        if (n == 1) return (int) w1;
        
        long w2 = ways(s.charAt(1)) * w1 + ways(s.charAt(0), s.charAt(1));
        for (int i = 2; i < n; i++) {
        	long temp = w2;
        	w2 = (ways(s.charAt(i)) * w2 + ways(s.charAt(i-1), s.charAt(i)) * w1) % (1000000000 + 7);
        	w1= temp;
        }
        return (int)w2;
        */
	...
	private int ways(char c1, char c2) {
		...
		else if (c1 == '*'  && c2 == '*') return 15;  // 11-19, 21-29
		else if (c1 == '*'){
