...
public class NumberOfLongestIncreasingSubsequence {
    public int findNumberOfLIS(int[] nums) {
        ...
        int[] len = new int[n]; // len[i] is the length of the longest
                                // increasing subsequence up to ith element
        int[] count = new int[n]; // count[i] is the count of the longest
                                  // increasing subsequences up to ith element
        int maxLen = 1, result = 0;

        // len[k+1] = max(len[k+1], len[i]+1) for all i <= k and nums[i] <
        // nums[k+1];
        // count[k+1] = sum(count[i]) for all i <= k and nums[i] < nums[k+1] and
        // len[i] = len[k+1]-1;
        for (int i = 0; i < n; i++) {
            len[i] = 1;
            count[i] = 1;
