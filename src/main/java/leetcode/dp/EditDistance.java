		...
		for (int i = 0; i < word1.length(); i++) {
			for (int j = 0; j < word2.length(); j++) {
				if (word1.charAt(i) == word2.charAt(j)) {
					...
					dp[i+1][j+1] = Integer.min(dp[i][j] + 1, // replace word1[i] with word[j]
							dp[i][j+1] + 1);  // delete word1[i] and convert word1(0...i-1) into word2(0...j) - dp[i][j+1]
					dp[i+1][j+1] = Integer.min(dp[i+1][j+1], dp[i+1][j]+1); //  insert at word1[i]
 				}
			}
		}
