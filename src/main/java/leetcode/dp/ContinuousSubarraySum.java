        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            for (int j = i + 1; j < nums.length; j++) {
                ...
                if (sum == k || (k != 0 && sum % k == 0))
                    return true; // need to consider k is 0
            }
        }
        return false;

        /*
         * // Space : O(n) int n = nums.length; int[] sum = new int[n]; sum[0] =
         * nums[0]; for (int i = 1; i < n; i++) { sum[i] = nums[i] + sum[i-1]; }
         * for (int start = 0; start < n -1; start++) { for (int end = start+1;
         * end < n; end++) { int s = sum[end] - sum[start] + nums[start]; if (s
         * == k || (k != 0 && s % k == 0)) return true; } } return false;
         */
    }
