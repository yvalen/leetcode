...
public class ScrambleString {
	public boolean isScramble_bottomUp(String s1, String s2) {
		...
		// dp[k][i][j] indicates whether s1[i...i+k] can be transformed from s2[j...j+k]
        ...
        for (int k = 2; k <= len; k++) {
        	for (int i = 0; i <= len-k; i++) {
        		for (int j = 0; j <= len-k; j++) {
        			...
        			for (int q = 1; q < k; q++) {
        				isScramble = (dp[q][i][j] && dp[k-q][i+q][j+q]) || (dp[q][i][j+k-q] && dp[k-q][i+q][j]) ;
        				if (isScramble) break;
        			}
        			dp[k][i][j] = isScramble;
        		}
	...
	public boolean isScramble_bottomUp2(String s1, String s2) {
		...
		// indicates whether the substring s1[i..i + k - 1] is a scramble of s2[j..j + k - 1]
		// Since each of these substrings is a potential node in the tree, we need to check for all possible cuts
		// F(i, j, k) = for some 1 <= q < k we have: (F(i, j, q) AND F(i + q, j + q, k - q)) OR (F(i, j + k - q, q) AND F(i + q, j, k - q))
	...
	public boolean isScramble_recursive(String s1, String s2) {
		...
		for (int i = 1; i < len; i++) {
			if (isScramble_recursive(s1.substring(0, i), s2.substring(0, i)) && isScramble_recursive(s1.substring(i), s2.substring(i))) {
				return true;
			}
			
			if (isScramble_recursive(s1.substring(0, i), s2.substring(len-i)) && isScramble_recursive(s1.substring(i), s2.substring(0, len-i))) {
				return true;
			}
		}
		return false;
