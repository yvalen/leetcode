 ...
 * 
 * http://www.drdobbs.com/parallel/finding-the-median-of-two-sorted-arrays/
 * 240169222
 */
public class MedianOfTwoSorted {
     ...
     * should be O(log (m+n)). Example 1: nums1 = [1, 3], nums2 = [2], the
     * median is 2.0 Example 2: nums1 = [1, 2], nums2 = [3, 4], the median is (2
     * + 3)/2 = 2.5
     * 
     * https://discuss.leetcode.com/topic/4996/share-my-o-log-min-m-n-solution-
     * with-explanation
     */
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        if (i == 0)
            ...
            maxOfLeft = Integer.max(nums1[i - 1], nums2[j - 1]);
        ...
        if (i == m)
            ...
            minOfRight = Integer.min(nums1[i], nums2[j]);
    ...
    private int findKthElement(int[] nums1, int[] nums2, int s1, int s2, int k) {
        ...
        // base case
        if (k == 1)
            return Integer.min(nums1[s1], nums2[s2]);
