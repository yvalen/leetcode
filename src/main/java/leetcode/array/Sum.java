...
public class Sum {
    /**
     * LEETCODE 1 Given an array of integers, return indices of the two numbers
     * such that they add up to a specific target. You may assume that each
     * input would have exactly one solution. Example: Given nums = [2, 7, 11,
     * 15], target = 9, Because nums[0] + nums[1] = 2 + 7 = 9, return [0, 1].
     * 
     * Company: LinkedIn, Uber, Airbnb. Facebook, Amazon, Microsoft, Apple,
     * Yahoo, Dropbox, Bloomberg, Yelp, Adobe Difficulty: easy Similar
     * Questions: 15(3sum), 18(4sum), 167(Two Sum II, sorted input array),
     * 653(TwoSumBST), 560(SubarraySumEqualsK), 170(TwoSum, data structure
     * design)
     */
    ...
    /**
     * LEETCODE 167 Given an array of integers that is already sorted in
     ...
     * numbers such that they add up to the target, where index1 must be less
     ...
     * exactly one solution and you may not use the same element twice. Input:
     * numbers={2, 7, 11, 15}, target=9 Output: index1=1, index2=2
     * 
     * Company: Amazon Difficulty: easy Similar Questions: 1(Two Sum),
     * 653(TwoSumBST)
     */
    ...
    /**
     * LEETCODE 15 Given an array S of n integers, are there elements a, b, c in
     * S such that a + b + c = 0? Find all unique triplets in the array which
     * gives the sum of zero. Note: The solution set must not contain duplicate
     * triplets. For example, given array S = [-1, 0, 1, 2, -1, -4], A solution
     * set is: [[-1, 0, 1], [-1, -1, 2]]
     * 
     * Company: Facebook, Microsoft, Bloomberg, Amazon, Adobe, Works
     * Applications Difficulty: medium Similar Questions: 1(2sum), 16(3sum
     * closest), 18(4sum), 259(ThreeSumSmaller)
     */
    public List<List<Integer>> threeSum(int[] nums) {
        ...
        for (int i = 0; i < nums.length - 2; i++) {
            ...
            for (int j = i + 1, k = nums.length - 1; j < k;) {
                ...
                if (sum == target) {
                    ...
                    // cannot break here as there might be other elements
        ...
        return result;
        /*
         * int n = nums.length; for (int i = 0; i < n; i++) { if (i > 0 &&
         * nums[i] == nums[i-1]) continue;
         * 
         * int j = i + 1, k = n - 1; while (j < k) { if (j > i+1 && nums[j] ==
         * nums[j-1]) { j++; continue; }
         * 
         * if (k < n-1 && nums[k] == nums[k+1]) { k--; continue; }
         * 
         * int sum = nums[i] + nums[j] + nums[k]; if (sum == 0) {
         * result.add(Arrays.asList(nums[i], nums[j], nums[k])); j++; k--; }
    ...
    public int threeSumClosest(int[] nums, int target) {
        ...
        for (int i = 0; i < nums.length - 2; i++) {
            for (int j = i + 1, k = nums.length - 1; j < k;) {
                ...
                if (diff < minDiff) { // update mindiff and closestsum along the
                                      // way
                    minDiff = diff;
                    closestSum = sum;
                }
         ...
         * Arrays.sort(nums);
         * 
         * for (int i = 0; i < n; i++) { if (i > 0 && nums[i] == nums[i-1])
         * continue;
         * 
         * int j = i + 1, k = n - 1; while (j < k) { if (j > i+1 && nums[j] ==
         * nums[j-1]) { j++; continue; }
         ...
         * if (k < n-1 && nums[k] == nums[k+1]) { k--; continue; }
         * 
         * int diff = nums[i] + nums[j] + nums[k] -target; if (diff == 0) {
         * return target; }
         * 
         * if (diff < 0) { j++; } else { k--; }
         * 
         * if (minDiff > Math.abs(diff)) { minDiff = Math.abs(diff); closetSum =
         * diff + target; } } }
         * 
         * return closetSum;
         */
    ...
    /**
     * LEETCODE 18 Given an array S of n integers, are there elements a, b, c,
    ...
    /**
     * LEETCODE 454 Given four lists A, B, C, D of integer values, compute how
     * many tuples (i, j, k, l) there are such that A[i] + B[j] + C[k] + D[l] is
     * zero. To make problem a bit easier, all A, B, C, D have same length of N
     * where 0 ≤ N ≤ 500. All integers are in the range of -2^28 to 2^28 - 1 and
     * the result is guaranteed to be at most 2^31 - 1. Example: Input: A = [ 1,
     * 2] B = [-2,-1] C = [-1, 2] D = [ 0, 2] Output: 2 Explanation: The two
     * tuples are: 1. (0, 0, 0, 1) -> A[0] + B[0] + C[0] + D[1] = 1 + (-2) +
     * (-1) + 2 = 0 2. (1, 1, 0, 0) -> A[1] + B[1] + C[0] + D[0] = 2 + (-1) +
     * (-1) + 0 = 0
     * 
     * Difficulty: medium Similar Questions: 18(4sum)
     */
