...
public class MissingRangers {
	...
	public List<String> findMissingRanges_long(int[] nums, int lower, int upper) {
		...
		if (lower < nums[0]) {
			sb.append(lower);
			if (nums[0] > lower+1) sb.append("->").append(nums[0]-1);
			result.add(sb.toString());
			sb.setLength(0);
		}
		...
		if (nums[nums.length-1] < upper) {
			if (upper > nums[nums.length-1] + 1) sb.append(nums[nums.length-1] + 1).append("->");
			sb.append(upper);
			result.add(sb.toString());
		}
	...
	public List<String> findMissingRanges(int[] nums, int lower, int upper) {
		...
		for (int num : nums) {
			...
			if (lower == num - 1) result.add(String.valueOf(lower));
			else if (lower < num-1) result.add(String.valueOf(lower) + "->" + String.valueOf(num-1));
			if (num == Integer.MAX_VALUE) return result;  // result when reaching max value
			lower = num + 1;  // make lower bigger than num
		}
		
		if (lower == upper) result.add(String.valueOf(lower));
		else if (lower<upper) result.add(String.valueOf(lower) + "->" + String.valueOf(upper));
		return result;
	}
