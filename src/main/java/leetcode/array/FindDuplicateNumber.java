...
public class FindDuplicateNumber {
    // At first the search space is numbers between 1 to n. Each time select a
    // number mid (which is the one in the middle) and
    // count all the numbers equal to or less than mid. If the count is more
    // than mid, the search space will be [1...mid]
    // otherwise [mid+1... n]. Do this until search space is only one number.
    // Proof
    // Let count be the number of elements in the range 1 .. mid
    // If count > mid, then there are more than mid elements in the range 1 ..
    // mid and thus that range contains a duplicate.
    // If count <= mid, then there are n+1-count elements in the range mid+1 ..
    // n. That is, at least n+1-mid elements
    ...
    public int findDuplicate_binarySearch(int[] nums) {
        ...
        while (lo < hi) {
            int mid = lo + (hi - lo) / 2;
            int count = 0; // count of numbers that are less than or equal to
                           // mid
            for (int num : nums) {
                if (num <= mid)
                    count++;
    ...
    public int findDuplicate_cycle(int[] nums) {
        // there is no 0 inside this array. You cannot access nums[0] from any
        // nums[i].
