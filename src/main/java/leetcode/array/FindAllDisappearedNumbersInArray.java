...
public class FindAllDisappearedNumbersInArray {
    public List<Integer> findDisappearedNumbers(int[] nums) {
        ...
        for (int num : nums) {
            ...
            // Subtracting by 1 is so that we can map all integers from 1 to n
            // using the
            // current array (array indexing starts at 0 so value 1 in the array
            // maps to 0 index etc)
            int idx = Math.abs(num) - 1;
            if (nums[idx] > 0)
                nums[idx] = -nums[idx]; // only flip the number when it is
                                        // positive
        }
