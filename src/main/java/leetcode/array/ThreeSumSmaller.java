...
public class ThreeSumSmaller {
	...
	public int threeSumSmaller(int[] nums, int target) {
		...
		int count = 0;
        for (int i = 0; i < nums.length-2; i++) { // cannot check for nums[i] <= target here because the following number may make the total smaller, i.e -4,-1,-1,0,1,2 target=5
		...
		for (int i = 0; i < n; i++) {
			...
			while (j < k) {
				...
				if (sum >= target) {
					...
					result += k - j; // all elements between k and j will satisfy 
					j++;
				}
			}
		}
