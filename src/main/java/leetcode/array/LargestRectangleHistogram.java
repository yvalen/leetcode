...
public class LargestRectangleHistogram {
    ...
    public int largestRectangleArea_bruteForce(int[] heights) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = i; j < n; j++) { // j should start from i to include
                                          // the individual bar
    ...
    // - the widest possible rectangle with height equal to the height of the
    // shortest bar.
    // - the largest rectangle confined to the left of the shortest
    // bar(subproblem).
    // - the largest rectangle confined to the right of the shortest
    // bar(subproblem).
    // Time complexity: O(nlogn) average O(n^2) worst case when array is sorted
    // Space complexity: O(n) for recursion
    // can be optimized with segment tree
    // https://discuss.leetcode.com/topic/45822/segment-tree-solution-just-another-idea-o-n-logn-solution/2
    ...
    private int divideAndConquerHelper(int[] heights, int start, int end) {
        ...
        return Integer.max(heights[minIdx] * (end - start + 1), Integer.max(
                divideAndConquerHelper(heights, start, minIdx - 1), divideAndConquerHelper(heights, minIdx + 1, end)));
    }
    ...
    // all the bars stored in the stack that has
    // height >= h will be right bounded by h (Remember we are calculating the
    // area with ‘x’ as the smallest bar in the rectangle.
    // - At that moment, since in the stack, we are maintaining a increasing
    // height in the stack, every time we check a bar that
    // is right bounded by h, will also be left bounded by the height that is
    // previous stored in the stack. So the width would go
    // from stack[-1] + 1 to i - 1 included, which is i - stack[-1] - 1
    // - If we are examing the last element in the stack, this means he is the
    // lowest till the leftmost of the histogram, so the width
