...
public class FirstMissingPositive {
    // For any k positive numbers (duplicated allowed), the first missing number
    // must be within [1, k+1]

    // Time Complexity: O(n) , O(1) space
    // consider position with A[i] = i+1 as a CORRECT SLOT.
    // - CORRECT SLOT will never be changed: for CORRECT SLOT, A[A[i] - 1]
    // always equals to A[i], vice versa (1)
    // - For each swap, the number of CORRECT SLOT increases by at least 1:
    // Position A[A[i] - 1] = A[i] becomes a CORRECT SLOT after swap,
    // and according to (1), this must be a new CORRECT SLOT
    // - The maximum of CORRECT SLOT <= N
    public int firstMissingPositive(int[] nums) {
        for (int i = 0; i < nums.length; i++) {
            // put each positive number k in range of i to n in its position
            // nums[k-1]
            int num = nums[i];
            while (num > 0 && num <= nums.length && nums[num - 1] != nums[i]) { // use
                                                                                // while
                                                                                // loop
                                                                                // here
                                                                                // instead
        ...
        // the first missing positive will be the position in which the number
        // doesn't match its index
        for (int i = 0; i < nums.length; i++) {
            if (nums[i] != i + 1)
                return i + 1;
        }
    ...
    public int firstMissingPositive_withPartition(int[] nums) {
        // partition nums such that all positive numbers are before k and
        // negative numbers are in k to n-1
