...
public class Intervals {
    /**
     * LEETCODE 57 Given a set of non-overlapping intervals, insert a new
     * interval into the intervals (merge if necessary). You may assume that the
     * intervals were initially sorted according to their start times. Example
     ...
     * 
     * Company: Google, Facebook, LinkedIn Difficulty: hard
     */
    ...
    /*
     * LEETCODE 56 Given a collection of intervals, merge all overlapping
     * intervals. For example, given [1,3],[2,6],[8,10],[15,18], return
     * [1,6],[8,10],[15,18].
     * 
     * Company: Google, Facebook, Microsoft, Bloomberg, LinkedIn, Twitter, Yelp
     * Difficulty: medium Similar Questions: 57(Insert Interval),
     * 252/253(MeetingRoom), 495(TeemoAttacking), 616
     */
    ...
    public List<Interval> merge(List<Interval> intervals) {
        ...
        /*
         * Interval merged = intervals.get(0); for (int i = 1; i <
         * intervals.size(); i++) { if (intervals.get(i).start > merged.end) {
         * // disjoint element, add merged to result set and reset merged
         * result.add(new Interval(merged.start, merged.end)); merged =
         * intervals.get(i); } else { // update end of merged merged.end =
         * Math.max(merged.end, intervals.get(i).end); } } result.add(merged);
         */
    ...
    public List<Interval> merge_withArray(List<Interval> intervals) {
        ...
        // for the result distinct Interval, the latter one's start must >
        // previous one's end.
