...
public class Vector2D implements Iterator<Integer>{
	...
	public boolean hasNext() {
		...
		return j == null ? false : j.hasNext(); // need to do null check for j for [] as input
	}
	...
	public boolean hasNext() {
		...
		while (!itr.hasNext() && index < size-1) { // need to loop here to handle empty list
			index++;
			itr = vec2d.get(index).iterator();
		}
	...
	*/
	...
	public static void main(String[] args) {
		//List<List<Integer>> vec2d = Arrays.asList(Arrays.asList(1, 2), Arrays.asList(3), Arrays.asList(4, 5, 6));
