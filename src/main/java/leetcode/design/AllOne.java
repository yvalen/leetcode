...
public class AllOne {
    ...
    // 2. keyCountMap maintains the count of keys, countBucketMap provides O(1) access to a specific Bucket with given count. 
	...
	private final Map<String, Integer> countMap;  // stores the key and its corresponding count
	private final Map<Integer, ListNode> nodeMap;  // stores the count and its corresponding node in the linked list
	...
	public void inc(String key) {
		if (countMap.containsKey(key)) {
			...
			if (head.next.count != 1) {
				insertNodeAfter(new ListNode(1), head);
    ...
    private void updateCount(String key, int offset) {
    	...
    	if (nodeMap.containsKey(newCount)) {
    		...
    		nodeMap.put(newCount, newNode); // need to add the newCount to nodeMap
    	}
