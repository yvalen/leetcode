...
public class VerifyBSTPreorderSequence {
    ...
    // Recursively examine every key in the array. For each BST node, its key
    // must be greater than
    // all keys in left subtree and less than keys in right subtree. Since given
    // preorder sequence,
    // the first element is always the root. Partition the array by the key of
    // root, find the index
    // of the first number greater than it.
    // Base case:
    // - start index exceeds end index, the array to be checked is empty, return
    // true;
    ...
    private boolean isValidPreorder(int[] preorder, int start, int end, int min, int max) {
        ...
        // cannot start from start+1 since this will not handle the left empty
        // tree
        int rightIdx = start;
    ...
    private boolean helper1(int[] preorder, int start, int end) {
        ...
        for (int i = start + 1; i <= end; i++) {
            ...
            if (rightIdx != -1 && preorder[i] < root) {
                return false; // all nodes in right tree should be greater than
                              // root
            }
        }
        ...
        return helper1(preorder, start + 1, rightIdx - 1) && helper1(preorder, rightIdx, end);
    }

    // keeping a stack of nodes (just their values) of which we're still in the
    // left subtree.
    // If the next number is smaller than the last stack value, then we're still
    // in the left
    // subtree of all stack nodes, so just push the new one onto the stack. But
    // before that,
    // pop all smaller ancestor values, as we must now be in their right
    // subtrees (or even further,
    // in the right subtree of an ancestor). Also, use the popped values as a
    // lower bound, since being
    // in their right subtree means we must never come across a smaller number
    // anymore.
