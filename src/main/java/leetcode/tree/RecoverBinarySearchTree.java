        ...
        while (!nodeStack.isEmpty() || node != null) {
        	if (node != null) {
        		...
        		if (prev != null && node.val < prev.val) {
        			...
        			// first element is the previous node that is greater than the current node
        			// second element is the current node that is smaller than the previous node 
        			if (first == null) {
        				first = prev;
        			}
        			
        			// always assign node to second to handle a tree with 2 elements
        			second = node;
        		}
        		prev = node;
        		node = node.right;
