...
public class ClosestBinarySearchTreeValue {
	...
	public int closestValue_recursive(TreeNode root, double target) {
		...
		// all nodes in right tree are greater than root, hence root will be closer to target
	...
	public List<Integer> closestKValues_inorder(TreeNode root, double target, int k) {
		Queue<Integer> queue = new LinkedList<>();  // keep the k elements, regular queue is enough because inorder sequence is sorted
        ...
        while (!stack.isEmpty() || node != null) {
        	if (node != null) {
        		...
        		if (queue.size() < k) {
        			...
        			// compare the diff of the first element in queue and target with node.val
	...
	// - Each call to findNextPred/Succ has amortized cost O(1), since each node is pushed/popped at most once. 
	// We have O(k) = O(n) calls of these, so these operations take O(n) in total.
	// According the follow up question, for a balanced BST, the size of "prec" and "suc" is bounded by logN. 
	// Each time, we call getPrec or getSuc, we may kind of shrink/enlarge the stack, but they are still bounded by logN. 
	// The amortized complexity will be O(1) for get, since we can consider it like "lazy traverse".
