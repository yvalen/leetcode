...
public class PathSum {
     ...
     * Company: Microsoft 
     * Difficulty: easy 
     * Similar Questions: 113(Path Sum II), 437(Path Sum III), 666(Path Sum IV), 
     * 129(SumRootToLeaf), 124(Binary Tree
     * Max Path Sum)
     */
    public boolean hasPathSum(TreeNode root, int sum) {
        ...
        // it needs to be a root-to-leaf path
     ...
     * Company: Bloomberg 
     * Difficulty: medium 
     * Similar Questions: 112(Path Sum), 437(Path Sum III), 666(PathSumIV), 
     * 257(BinaryTreePath)
     */
     ...
     * only from parent nodes to child nodes). The tree has no more than 1,000 nodes 
    ...
    public int pathSumIII_dfs(TreeNode root, int sum) {
        ...
        // apply the function instead of helper so that each child can be used as
        // a starting node
        return pathSumIII_dfs_helper(root, sum) + 
                pathSumIII_dfs(root.left, sum) + 
    ...
    // current node. We can imagine a path from the root to the current node. 
    ...
    // target value, which means the difference discussed in 2 should equal to 
    // the target value. In addition, we need to know how many differences are 
    // equal to the target value.
    // 4. Here comes the map. The map stores the frequency of all possible sum
    // in the path to the current node. If the difference between the current 
    // sum and the target value exists in the map, there must exist a node in 
    // the middle of the path, such that from this node to the current node, 
    ...
    // to calculate the number of ranges that sum up to target. Note that each 
    ...
    private int prefixSumHelper(TreeNode current, int currentSum, int sum, Map<Integer, Integer> prefixSumMap) {
        ...
        // difference between the sum from the root to the current node and the 
        // prefix sum of the node in the middle.
        // get this before updating prefixSumMap because currentSum-sum may equal to currentSum
    ...
    /**
     * LEETCODE 124 
     * Given a binary tree, find the maximum path sum. For this problem, a path 
     * is defined as any sequence of nodes from some starting node to any node 
     * in the tree along the parent-child connections. The path must contain at 
     ...
     */
    // A path from start to end, goes up on the tree for 0 or more steps, then goes down 
    // for 0 or more steps. Once it goes down, it can’t go up. Each path has a highest node, 
    ...
    private int maxPathSumDown(TreeNode root) {
        ...
        // don't use max from left if it is negative as it will make
        // the sum smaller
        int leftMaxSum = Math.max(0, maxPathSumDown(root.left)); 
        int rightMaxSum = Math.max(0, maxPathSumDown(root.right));
        // compare the new sum with the max sum obtained so far
        maxSum = Integer.max(maxSum, root.val + leftMaxSum + rightMaxSum); 
        // return the branch with higher sum, prune the branch with lower sum
