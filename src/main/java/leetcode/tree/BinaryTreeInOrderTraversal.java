...
public class BinaryTreeInOrderTraversal {
    ...
    /**
     * Morris traversal algorithm: use the right child pointer to thread the
     * tree 1. Initialize current as root 2. While current is not NULL If
     * current does not have left child a) Print current’s data b) Go to the
     * right, i.e., current = current->right Else a) Make current as right child
     * of the rightmost node in current's left subtree b) Go to this left child,
     * i.e., current = current->left Although the tree is modified through the
     * traversal, it is reverted back to its original shape after the
     * completion.
     * 
     * Threaded tree: https://en.wikipedia.org/wiki/Threaded_binary_tree A
     * binary tree is threaded by making all right child pointers that would
    ...
    public List<Integer> inorderMorrisTraversal(TreeNode root) {
        ...
        while (current != null) {
            if (current.left == null) {
                ...
                // find the inorder predecessor of current (rightmost child of
                // the left subtree)
                prev = current.left;
                ...
                if (prev.right == null) {
                    // find the rightmost child of the left subtree, make
                    // current as its right child
                    prev.right = current;
