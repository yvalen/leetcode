...
public class BinaryTreePostOrder {
	 ...
	 * then keep going down the tree, and add left/right node to stack when applicable. 
	...
	public List<Integer> postorderTraversal_iterative_withOneStack(TreeNode root) {
		...
		while (!nodeStack.isEmpty()) {
			...
			if (prev == null // handle root node 
					|| prev.left == current || prev.right == current // traverse down if prev is current's parent
					) { 
	...
	/*
	 * Doing a reversed pre-order traversal. That is, the order of traversal is a node, 
	 * then its right child followed by its left child. This yields post-order traversal 
	 * in reversed order. Using a second stack, we could reverse it back to the correct order.
	 * 
