 ...
 * Difficulty: easy
 * Similar Questions: 1(2sum), 167(Two Sum II, sorted input array), 
...
public class TwoSumBST {
    ...
    //
    // binary search: For each node, we check if k - node.val 
    // exists in this BST.
    ...
    private boolean find(TreeNode root, TreeNode current, int k) {
        ...
        // should subtract current.val from k instead of root.val for search
        // as current is the node checking right now
        // should use current.left and current.right in find
        return search(root, current, k - current.val) || 
                find(root, current.left, k) || // should always check from root
                                               // consider a case [2,1,3] k=4
    ...
    private boolean search(TreeNode root, TreeNode current, int target) {
        ...
        if (root.val == target) {
            // need to check if root is the current node
            return root != current;
        }
