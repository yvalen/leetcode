...
public class TwoSumBST {
    ...
    //
    // binary search: For each node, we check if k - node.val exists in this
    // BST.
    ...
    private boolean find(TreeNode root, TreeNode current, int k) {
        if (root == null || current == null)
            return false;
        return search(root, current, k - current.val) || // should subtract
                                                         // current.val from k
                                                         // instead of root.val
                                                         // as current is the
                                                         // node checking right
    ...
    private boolean search(TreeNode root, TreeNode current, int target) {
        ...
        if (root.val == target && root != current)
            return true; // need to check if root is the current node
