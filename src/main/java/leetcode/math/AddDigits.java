...
public class AddDigits {
    ...
    /*
     * https://en.wikipedia.org/wiki/Digital_root#Congruence_formula The digital
     * root (aka repeated digital sum) of a non-negative integer is the (single
     * digit) value obtained by an iterative process of summing digits, on each
     * iteration using the result from the previous iteration to compute a digit
     * sum. The process continues until a single-digit number is reached.
     * 
     * For base b (decimal case b = 10), the digit root of an integer is: dr(n)
     * = 0 if n == 0 dr(n) = (b-1) if n != 0 and n % (b-1) == 0 dr(n) = n mod
     * (b-1) if n % (b-1) != 0 or dr(n) = 1 + (n - 1) % 9
     */
