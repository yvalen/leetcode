...
public class StrobogrammaticNumber {
    ...
    public boolean isStrobogrammatic(String num) {
        for (int i = 0, j = num.length() - 1; i <= j; i++, j--) {
            ...
            // if (!map.containsKey(left) || map.get(left) != right) return
            // false;
        }
    ...
    private List<String> findStrobogrammaticHelper(int n, int size) {
        if (n <= 0)
            return Arrays.asList(""); // return a list with an empty string so
                                      // that the recursion would work
        ...
        for (String s : subList) {
            if (n < size)
                result.add("0" + s + "0"); // compare n with original size to
                                           // decide whether to use 0, 0 cannot
    ...
    private int inRangeHelper(String low, String high, int left, int right, char[] c) {
        ...
        for (char[] pair : pairs) {
            ...
            c[right] = pair[1];
            if (c.length != 1 && c[0] == '0')
                continue; // 0 can only be used in number with one digit
            if (left == right && pair[0] != pair[1])
                continue; // one digit in the middle cannot be 6 or 9
