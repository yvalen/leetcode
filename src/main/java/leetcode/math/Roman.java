...
public class Roman {
	...
	public int romanToInt(String s) {
		...
		// walk the string from back to front, if one letter is less than its latter one, this letter is subtracted;
		...
		int result = 0;
		char prev = '#'; 
		// the last letter is always added. walk the string from back to front, 
		// if one letter is less than its latter one, this letter is subtracted.
		for (int i = s.length() - 1; i >= 0; i--) {
			char c = s.charAt(i);
			int val = map.get(c);
			if (val < result && c != prev) {
				result -= val; // only do subtraction when current character is different from the following character, "DCXXI"
			}
			else {
				result += val;
			}
