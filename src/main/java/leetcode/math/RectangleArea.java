...
public class RectangleArea {
	...
	public int computeArea(int A, int B, int C, int D, int E, int F, int G, int H) {
		...
		int right = Math.max(Math.min(C,  G), left); // need to compare with left for the non-overlapping case
		int bottom = Math.max(B, F);
		int top = Math.max(Math.min(D, H), bottom);
		
		// no need to check if two rectangles overlap since the overlapping area will be zero if overlapping does not exist.
