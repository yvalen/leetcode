...
public class SmallestRectangleEnclosingBlackPixels {
    ...
    // DFS
    // Search starting from the given pixel. Since all the black pixels are
    // connected,
    ...
    public int minArea_binarySearch(char[][] image, int x, int y) {
        ...
        int left = searchColumns(image, 0, y, 0, m, true); // search col[0...y],
                                                           // find first column
                                                           // contain 1,
        int right = searchColumns(image, y + 1, n, 0, m, false); // search
                                                                 // col[y+1,
                                                                 // col], find
                                                                 // first column
                                                                 // contain all
                                                                 // 0
        int top = searchRows(image, 0, x, left, right, true); // search row
                                                              // [0...x], find
                                                              // first row
                                                              // contain 1,
        int bottom = searchRows(image, x + 1, m, left, right, false); // search
                                                                      // row[x+1,
                                                                      // row],
                                                                      // find
                                                                      // first
    ...
    private int searchColumns(char[][] image, int lo, int hi, int top, int bottom, boolean whiteToBlack) {
        while (lo < hi) {
            ...
            while (k < bottom && image[k][mid] == '0')
                k++;
            if ((k < bottom) == whiteToBlack) { // k < bottom means column mid
                                                // has a black pixel
                hi = mid;
            } else {
                lo = mid + 1;
    ...
    private int searchRows(char[][] image, int lo, int hi, int left, int right, boolean whiteToBlack) {
        while (lo < hi) {
            ...
            while (k < right && image[mid][k] == '0')
                k++;
            if ((k < right) == whiteToBlack) { // k < bottom means row mid has a
                                               // black pixel
                hi = mid;
            } else {
                lo = mid + 1;
