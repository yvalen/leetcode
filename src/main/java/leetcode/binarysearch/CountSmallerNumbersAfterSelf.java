...
public class CountSmallerNumbersAfterSelf {
    ...
    // array of numbers have been visited.
    // Use findInsertPosition() to find the FIRST element in the sorted array
    // which is larger or EQUAL to target number.
    // For example, [5,2,3,6,1], when we reach 2, we have a sorted array[1,3,6],
    // findIndex() returns 1, which is the
    // index where 2 should be inserted and is also the number smaller than 2.
    // Then we insert 2 into the sorted array
    // to form [1,2,3,6].
    ...
    private int findInsertPosition(List<Integer> sorted, int target) {
        ...
        if (sorted.get(start) >= target)
            return start; // should check for equal here because we need to
    ...
    // Binary Search Tree
    // build a binary search tree, Node class contains the value as well as how
    // many elements are less than that number
    // iterate through the nums array from right to left, every node will
    // remember all lesser values we have already processed.
    // Thus, by a result of your tree traversal while you insert, you can build
    // the result you want to return.
    // Time Complexity: O(nlogn) avaerage case, o(n^2) worst case when nums is
    // sorted
    ...
    private Node insert(Node node, int val, int index, int count, Integer[] result) { // count
                                                                                      // is
                                                                                      // the
                                                                                      // current
                                                                                      // count
        ...
        if (node == null) {
            ...
            node.right = insert(node.right, val, index, count + node.lessThan + (node.val == val ? 0 : 1), result); // only
                                                                                                                    // add
                                                                                                                    // 1
                                                                                                                    // to
                                                                                                                    // count
    ...
    // merge sort
    // The smaller numbers on the right are exactly those that jump from its
    ...
    public List<Integer> countSmaller_mergeSort(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++)
            sorted[i] = new ValueIndex(nums[i], i);
    ...
    private void mergesort(ValueIndex[] aux, ValueIndex[] sorted, int[] count, int lo, int hi) {
        ...
        for (int k = lo; k <= hi; k++)
            aux[k] = new ValueIndex(sorted[k]);

        for (int i = lo, j = mid + 1, k = lo; k <= hi; k++) {
            if (j > hi || (i <= mid && aux[i].val <= aux[j].val)) { // need to
                                                                    // check for
                                                                    // equal
                                                                    // here so
                                                                    // that we
                                                                    ...
                                                                    // increment
                                                                    // for dups
                // Each time we choose a left to the merged array, we want to
                // know how many numbers
                // from the right are moved before this number.
                sorted[k] = aux[i];
                count[aux[i].index] += j - (mid + 1);
                i++;
            } else { // only take item from right if it is smaller, for equal
                     // items take from left to preserve the order so that it is
                     // stable sort
                sorted[k] = aux[j];
                j++;
            }
