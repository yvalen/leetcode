...
public class ReconstructItinerary {
	 ...
	 * get stuck at any vertex other than v, because the even degree of all vertices ensures that, when the trail enters another 
	 * vertex w there must be an unused edge leaving w. The tour formed in this way is a closed tour, but may not cover all the 
	 ...
	 * The idea is to keep following unused edges and removing them until we get stuck. Once we get stuck, we back-track to the nearest vertex 
	 * in our current path that has unused edges, and we repeat the process until all the edges have been used
	 */
	 ...
	 * we check if we have used all our airline tickets. If not, we revert the change and try another ticket. We keep trying 
	...
	public List<String> findItinerary(String[][] tickets) {
        ...
        // start from "JFK", apply Hierholzer's algorithm to find a Eulerian path in the graph which is a valid reconstruction.
        dfs(graph, result, "JFK");
        return result;
    }
	
	private void dfs(Map<String, PriorityQueue<String>> graph, LinkedList<String> path, String from) {
		// base case, from is not the starting point of any itinerary, we have reached end point 
