...
public class CourseSchedule {
	 ...
	 * Difficulty: medium
	 * Similar Questions: 261(GraphValidTree), 310(MinumumHeightTree) 
	 */
	...
	public boolean canFinish_dfs(int numCourses, int[][] prerequisites) {
        ...
        boolean[] onStack = new boolean[numCourses]; // true while its edges are being processed
        for (int i = 0; i < numCourses; i++) {
	...
	private boolean dfs(List<List<Integer>> graph, boolean[] visited, boolean[] onStack, int course) {
		if (onStack[course]) return false;
		
		if (!visited[course]) {
			onStack[course] = true;
			List<Integer> dependencies = graph.get(course);
			for (Integer d : dependencies) {
				if (!dfs(graph, visited, onStack, d)) return false;
			}
	 ...
	 * Difficulty: medium
	 * Similar Questions: 269(AlienDictionary), 310(MinumumHeightTree), 444(SequenceReconstruction) 
	 */
	...
	public int[] findOrder_dfs(int numCourses, int[][] prerequisites) {
		...
		for (int i = 0; i < numCourses; i++) {
			if (!findOrder_dfsHelper(i, visitStatus, graph, stack)) { // has cycle
				return new int[0];
			}
		}
	...
	private boolean findOrder_dfsHelper(int course, int[] visitStatus, List<List<Integer>> graph, Stack<Integer> stack) {
		...
		for (Integer d : graph.get(course)) {
			if (!findOrder_dfsHelper(d, visitStatus, graph, stack)) return false;
		}
