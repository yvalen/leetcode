...
public class AlienDictionary {
	...
	// Algorithm https://courses.cs.washington.edu/courses/cse326/03wi/lectures/RaoLect20.pdf
	...
	public String alienOrder_indegree(String[] words) {
		...
		for (int i = 0; i < words.length - 1; i++) {
			...
			for (int j = 0; j < Math.min(current.length(), next.length()); j++) {
				// don't need to check every character here since indegree has been populated
	...
	public String alienOrder_dfs(String[] words) {
		if (words == null || words.length == 0) return "";
		if (words.length == 1) return words[0]; // if there is one word, order can by any combination
		
		Map<Character, Set<Character>> graph = new HashMap<>();
		for (int i = 1; i < words.length; i++) {
			String prev = words[i-1], current = words[i];
			boolean found = false;  // flag indicating whether we need to compare the two words
			for (int j = 0; j < Math.max(prev.length(), current.length()); j++) {
				...
				if (j < prev.length() && j < current.length() && 
						prev.charAt(j) != current.charAt(j) && !found) {
					graph.get(prev.charAt(j)).add(current.charAt(j)); // add the edge to graph
					found = true; // cannot compare the rest since their order are undeterministic
				}
			}
		}
		...
		for (Character v : graph.keySet()) {
			if (!visited[v-'a']) {
				if (!dfs(graph, v, sb, visited, new boolean[26])) return "";
			}
		}
		return  sb.reverse().toString();  
	}

	private boolean dfs(Map<Character, Set<Character>> graph, Character v, StringBuilder sb, 
		...
		if (graph.get(v) != null) {
			for (Character neighbor : graph.get(v)) {
				if (!visited[neighbor-'a']) {
					if (!dfs(graph, neighbor, sb, visited, onStack)) return false;
				}
