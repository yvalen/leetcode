	...
	public boolean sequenceReconstruction_bfs(int[] org, List<List<Integer>> seqs) {
		...
		for (List<Integer> seq : seqs) {
			for (int i = 0; i < seq.size(); i++) {
				...
				inDegrees.putIfAbsent(v, 0); // set to 0 only if the vertex never appears before, it may be on other's adjacent list 
				if (i > 0) {
		...
		if (org.length != inDegrees.size()) return false;
		...
		int index = 0;
		while(!queue.isEmpty()) {
			if (queue.size() > 1) return false;  // there will be multiple answers when queue size is greater than 1
			int current = queue.poll();
			if (org[index++] != current) return false;
