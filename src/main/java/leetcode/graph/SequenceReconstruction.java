...
public class SequenceReconstruction {
    ...
    // BFS
    public boolean sequenceReconstruction_topologicalSort(int[] org, List<List<Integer>> seqs) {
        ...
        for (List<Integer> seq : seqs) {
            for (int i = 0; i < seq.size(); i++) {
                ...
                // set to 0 only if the vertex never appears before
                // it may be on other's adjacent list or it may have no incoming edges
        ...
        while (!queue.isEmpty()) {
            if (queue.size() > 1) {
                // there will be multiple answers when queue size is greater than 1
                return false; 
            }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(sr.sequenceReconstruction_topologicalSort(org, seqs));
    }
}
