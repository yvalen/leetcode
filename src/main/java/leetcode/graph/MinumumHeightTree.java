...
public class MinumumHeightTree {
    ...
    private int getHeight(List<List<Integer>> graph, boolean[] visited, int root) {
        ...
        for (int neighbor : graph.get(root)) {
            if (!visited[neighbor]) {
                // height of each individual tree is the length of the path to
                // the deepest leaf
    ...
    public List<Integer> findMinHeightTrees(int n, int[][] edges) {
        ...
        while (n > 2) {
            ...
            for (Integer leaf : leaves) {
                ...
                graph.get(neighbor).remove(leaf); // cannot use primitive type
                                                  // for leaf since it will be
                                                  // treated as remove at index
