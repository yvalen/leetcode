...
public class RemoveDuplicateLetters {
	...
	public String removeDuplicateLetters_withMap(String s) {
		...
		for (int i = 0; i < result.length; i++) {
			...
			for (int j = start; j <= end; j++) {
				char c = s.charAt(j);
				if (indexMap.containsKey(c) && c < minChar) { // only need to consider char still in the map as the ones that have been processed are removed from the map
					minChar = c;
			...
			if (s.charAt(end) == minChar) end = getMinIndex(indexMap);
		}
	...
	public String removeDuplicateLetters_withStack(String s) {
        ...
        int[] occurrenceCount = new int[26]; // number of times the char appears in the string
        for (char c : charArray) {
        	occurrenceCount[c-'a']++;
        }
        
        Deque<Character> deque = new ArrayDeque<>(); 
        boolean[] visited = new boolean[26]; // whether the char has been processed - in the stack 
        for (char c : charArray) {
        	int idx = c - 'a';
        	occurrenceCount[idx]--; // decrement the occurrence count
        	if (visited[idx]) continue; // skip the char that's already in stack
        	
        	// current char is smaller than the top of the stack and the top of stack exists in s in a later position
        	...
        	while (!deque.isEmpty() && c < deque.peekLast() && occurrenceCount[deque.peekLast()-'a'] > 0) {
        		visited[deque.pollLast()-'a'] = false; // pop the larger char and set the visited flag to false so that it can be processed later
        	}
        	deque.offerLast(c);
        	visited[idx] = true;
        }
	...
	// Greedy
	//
	// Given the string s, the greedy choice (i.e., the leftmost letter in the answer) is the smallest s[i], such that
