...
public class GasStation {
    ...
    // The tank should never be negative, so restart whenever there is a
    // negative number.
    ...
    // every time we start from a station, we go as far as possible by
    // increasing end until remaining gas is less than 0.
    // If end finally hits start we know we can travel around from start. If we
    // haven't traveled around, we know we cannot
    // start from this station. Then we check the station before our start
    // station if we can start from this station.
