...
public class RearrangeStringKDistanceApart {
    public String rearrangeString_heap(String s, int k) {
        ...
        /*
         * Map<Character, Integer> countMap = new HashMap<>(); for (Character c
         * : s.toCharArray()) { countMap.put(c, countMap.getOrDefault(c, 0)+1);
         * }
         * 
         * PriorityQueue<Map.Entry<Character, Integer>> pq = new
         * PriorityQueue<>(countMap.size(), (a, b) -> a.getValue()==b.getValue()
         * ? a.getKey().compareTo(b.getKey()) : (b.getValue() - a .getValue()));
         * pq.addAll(countMap.entrySet()); Queue<Map.Entry<Character, Integer>>
         * waitq = new LinkedList<>();
         * 
         * StringBuilder sb = new StringBuilder(); while (!pq.isEmpty()) {
         * Map.Entry<Character, Integer> entry = pq.poll();
         * entry.setValue(entry.getValue()-1); sb.append(entry.getKey());
         * waitq.offer(entry);
         * 
         * if(waitq.size() < k) continue; // waitq is not full yet
         * 
         * // release from waitq if char is already k chars apart entry =
         * waitq.poll(); if (entry.getValue() > 0) pq.offer(entry); }
         * 
         * return sb.length() == s.length() ? sb.toString() : "";
         */
    }
    ...
    public String rearrangeString(String s, int k) {
        ...
        int[] position = new int[26]; // stores the next valid position for the
                                      // char
    ...
    // select a candidate character which has most remain count and is allowed
    // at current
