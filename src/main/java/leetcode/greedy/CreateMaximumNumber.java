...
public class CreateMaximumNumber {
    ...
    public int[] maxNumber(int[] nums1, int[] nums2, int k) {
        ...
        // divide the k digits required into two parts, i and k-i. then find the
        // maximum number
        // of length i in one array and the maximum number of length k-i in the
        // other array
    ...
    // - Loop through the array nums, pop the top of stack if it is smaller than
    // nums[i] until stack is empty
    // or the digits left is not enough to fill the stack to size k. if stack
    // size < k push nums[i]
    // Return stack
    int[] getMaxNumberWithKDigitsWithStack(int[] nums, int k) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if (stack.size() < k)
                stack.push(nums[i]); // only add to stack when stack size is
                                     // less than k
        }
