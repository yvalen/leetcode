...
public class JumpGame {
    ...
    // potential jump that
    // reaches a GOOD index (currPosition + nums[currPosition] >=
    // leftmostGoodIndex).
    // If we can reach a GOOD index, then our position is itself GOOD. Also,
    // this new GOOD position
    // will be the new leftmost GOOD index. Iteration continues until the
    // beginning of the array.
    // If first position is a GOOD index then we can reach the last index from
    // the first position.
    ...
    // Try every jump pattern from first position to last. Start from the first
    // position
    // and jump to every index that is reachable until the last index is
    // reached. Backtrack when stuck
    ...
    private boolean backtrackHelper(int position, int[] nums) {
        ...
        for (int nextPosition = position + 1; nextPosition <= furthestPosition; nextPosition++) {
            if (backtrackHelper(nextPosition, nums))
                return true;
        }
        return false;
    }
    ...
    public int jump_greedy(int nums[]) {
        ...
        for (int i = 0; i < nums.length - 1; i++) {
            // currentFurthest is the farthest point that all points before
            // currentEnd can reach
