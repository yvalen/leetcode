...
public class ShortesDistanceFromAllBuildings {
    ...
    public int shortestDistance(int[][] grid) {
        ...
        int[][] distances = new int[m][n]; // stores the sum of the shortest
                                           // distances from this cell to all
                                           // reachable buildings
        int[][] reachableBuildings = new int[m][n]; // stores the number of
                                                    // buildings reachable from
                                                    // this cell
        int numOfBuildings = 0;

        // for each building, use BFS to calculate the shortest distance from
        // each 0 to it
    ...
    private void bfs(int[][] grid, int row, int col, int[][] distances, int[][] reachableBuildings) {
        ...
        while (!queue.isEmpty()) {
            ...
            // can not use the distances of the current cell because it is
            // updated for each traversal
            int size = queue.size();
            while (size-- > 0) {
                ...
                for (int[] direction : DIRECTIONS) {
                    ...
                    if (x < 0 || x >= grid.length || y < 0 || y >= grid[0].length || grid[x][y] != 0 || visited[x][y]) {
                        // visited is local for each iteration. If it is true it
                        // means this cell has been checked already for this
                        // building
                        continue;
                    }
                    reachableBuildings[x][y]++;
                    visited[x][y] = true;
                    distances[x][y] += level; // need to add on top of
                                              // distances[x][y] to keep track
                                              // of distance to all buildings
    ...
    // In BFS walk only onto the cells that were reachable from all previous
    // buildings.
    // From the first building only walk onto cells where grid is 0, and make
    // them -1.
    ...
    public static void main(String[] args) {
        ...
        /*
         * int row = 0, col =0, m = 3, n = 5; int[][] distances = new int[m][n];
         * int[][] reachableBuildings = new int[m][n]; sdab.bfs(grid, row, col,
         * distances, reachableBuildings); MatrixUtil.print(distances);
         * System.out.println(); MatrixUtil.print(reachableBuildings);
         */
