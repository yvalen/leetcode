...
public class IslandPerimeter {
	...
	// Each land cell has 4 sides and to get the total potential number of sides you would need to multiply the total number of lands by 4. 
	// A side which is not included in the perimeter count is a side that belongs to two islands.
	// skip left or up neighbor because they must have been counted by other cells as their right or down neighbors, 
	public int islandPerimeter(int[][] grid) {
        ...
        for (int i = 0; i < grid.length; i++) {
        	for (int j = 0; j < grid[0].length; j++) {
        		if (grid[i][j] == 1) {
        			lands++;
        			if (i < grid.length-1 && grid[i+1][j] == 1) neighbors++; // count down neighbor
        			if (j < grid[0].length-1 && grid[i][j+1] == 1) neighbors++; // count right neighbors
        		}
        	}
        }
