...
public class NestedListWeightSum {
	 ...
	 * Company: LinkedIn
	 * Difficulty: easy
	 * Similar Questions: 690(EmployeeImportance)
	 */
    ...
    public int depthSum_bfs(List<NestedInteger> nestedList) {
        ...
        while (!queue.isEmpty()) {
            int size = queue.size();
            while (size-- > 0) {
                List<NestedInteger> current = queue.poll();
                for (NestedInteger ni : current) {
                    if (ni.isInteger()) sum += ni.getInteger() * level;
                    else queue.offer(ni.getList());
                }
            }
            level++;
        }
    ...
    public int depthSumInverse_bfs(List<NestedInteger> nestedList) {
        ...
        while (nestedList.size() > 0) {
        	...
        	for (NestedInteger ni : nestedList) {
        		if (ni.isInteger()) unweighted += ni.getInteger(); // add on top of existing value of unweighted, this guarantees the outer layer element being added to the sum in terms of their layers
