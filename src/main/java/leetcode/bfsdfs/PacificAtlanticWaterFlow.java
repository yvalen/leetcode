 ...
 * touches the left and top edges of the matrix and the "Atlantic ocean" touches the right and bottom edges. Water can only flow 
 * in four directions (up, down, left, or right) from a cell to another one with height equal or lower. Find the list of grid coordinates 
...
public class PacificAtlanticWaterFlow {
	...
	public List<int[]> pacificAtlantic_bfs(int[][] matrix) {
        ...
        for (int i = 0; i < m; i++) {
        	for (int j = 0; j < n; j++) {
        		if (visitedP[i][j] && visitedA[i][j]) result.add(new int[] {i, j});
        	}
        }
        return result;
    }
	...
	private void dfs(int[][] matrix, boolean[][] visited, int[] start) {
		...
		for (int[] dir : DIRS) {
			...
			if (x >= 0 && x < matrix.length && y >= 0 && y < matrix[0].length && !visited[x][y] && 
					matrix[x][y] >= matrix[row][col]) { // starting point should be the lowest on height
