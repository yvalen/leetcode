...
public class ZeroOneMatrix {
	...
	public int[][] updateMatrix_bfs(int[][] matrix) {
		...
		while (!queue.isEmpty()) {
			...
			for (int[] delta : DELTAS) {
				...
				if (x < 0 || x >= m || y < 0 || y >= n || 
						matrix[x][y] == 0 || result[x][y] != 0) { // use result array to check if the cell has been visited
					continue;
				}
	...
	public int[][] updateMatrix_dfs(int[][] matrix) {
		...
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				if (matrix[i][j] == 1) result[i][j] = Integer.MAX_VALUE;
			}
		}
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
	...
	private void dfs(int[][] matrix, int[][] result, int row, int col, int distance) {
		...
		result[row][col] = distance;
		for (int[] delta : DELTAS) {
			dfs(matrix, result, row + delta[0], col + delta[1], distance + 1);
		}
	}
*/
