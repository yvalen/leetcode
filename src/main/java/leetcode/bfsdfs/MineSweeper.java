...
public class MineSweeper {
    ...
    public char[][] updateBoard_dfs(char[][] board, int[] click) {
        ...
        for (int[] dir : DIRS) {
            ...
            if (x >= 0 && x < m && y >= 0 && y < n) {
                if (board[x][y] == 'M' || board[x][y] == 'X') {
                    // increment count if surrounding cell has either a
        ...
        // need to mark the cell as a revealed empty cell before recursion
        board[row][col] = 'B'; 

        // perform dfs for unrevealed surrounding cells
        for (int[] dir : DIRS) {
            int x = row + dir[0], y = col + dir[1];
            // only perform dfs for un-revealed cells
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] dir : DIRS) {
                ...
                if (x >= 0 && x < m && y >= 0 && y < n && board[x][y] == 'E') {
                    ...
                    // when enqueue an empty square, need to mark it as revealed 
                    // this is to prevent this cell to be added again
                    board[x][y] = 'B'; 
                }
            }
        }
