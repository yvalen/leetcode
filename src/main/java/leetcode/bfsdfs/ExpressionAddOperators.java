...
public class ExpressionAddOperators {
	...
	public void dfs(String num, int target, List<String> result, String path, int position, long currentResult, long prev) { // use long for currentResult for prevent overflow
		...
		for (int i = position; i < num.length(); i++) {
			if (i != position && num.charAt(position) == '0') { // handle 0 sequence, we cannot have multiple digits starting with 0
				break;
			}
			long currentNum = Long.parseLong(num.substring(position, i+1)); // get the current number to process, combination of digits after position (inclusive)
			if (position == 0) { // don't prepend operator for the first digit
				...
				// multiply op
				dfs(num, target, result, path+"*"+currentNum, i+1, currentResult-prev+prev*currentNum, prev*currentNum);
			}
		}
	}
