...
public class Reverse {
	 ...
	 * 
	 * Difficulty: easy
	 * Similar Questions: 345(Reverse Vowels of a String), 541(Reverse String II)
	 */
	...
	public String reverseStr(String s, int k) {
        ...
        while (start < end && start < s.length()) {
        	if ((start/k) % 2 == 0) reverse(chars, start, end);
        	start += k;
	 ...
	 * Company: Zappos
	 * Difficulty: easy
	 * Similar Questions: 541(Reverse String II)
	 */
	public String reverseWordsIII(String s) {
        ...
        int i = 0, j = 0;
        while (j < s.length()) {
        	while (j < s.length() && chars[j] != ' ') { // need to do the length check first, otherwise will get index out of bound error
        		j++;
        	}
        	reverse(chars, i, j-1); // end index should be j-1 since j points to white space
        	i = j + 1;
        	j++;
        }
        return new String(chars);
	 ...
	 * - The input string may contain leading or trailing spaces. But the reversed string should not contain leading or trailing spaces.
	 * - There could be multiple spaces between words in the input string. The reversed string should contain a single space bwteen words.
	 * 
	 * Company: Microsoft, Bloomberg, Apple, Snapchat, Yekp
	 * Difficulty: medium
	...
	public String reverseWords_withCharArray(String s) {
        ...
        j = 0;
        while (j < n) {
        	while (j < n && chars[j] == ' ') j++; // skip space
        	while (j < n && chars[j] != ' ') chars[i++] = chars[j++]; // keep no space
        	while (j < n && chars[j] == ' ') j++; // skip space
        	if (j < n) chars[i++] = ' ';  // add a single space between words
        }
	...
	public String reverseWords_withStringBuilder(String s) {
        ...
        sb.deleteCharAt(sb.length()-1); // should use sb.length instead of s.length since there could spaces removed from the middle
        return sb.toString();
	}
	 ...
	 * Difficulty: medium
	 * Similar Questions: 151(Reverse Words in a String), 189(RotateArray)
	 */
	...
	public String reverseVowels(String s) {
		...
		for (int i = 0, j = s.length() - 1; i < j;) {
            while (i < j && !isVowel(chars[i])) { // need to check i < j instead if i < s.length() 
            	i++;
            }
            while (i < j && !isVowel(chars[j])) { // need to check i < j instead if j >= 0
            	j--;
            }
            if (i == s.length() || j < 0) break;  // break if index is out of bound
            char c = chars[i];
            chars[i] = chars[j];
            chars[j] = c;
            i++;
