...
public class SolveEquation {
	...
	public String solveEquation(String equation) {
		if (equation == null || equation.isEmpty()) return NO_SOLUTION;
		
		String[] parts = equation.split("=");
		if (parts.length != 2) return NO_SOLUTION;
		
		int[] left = evaluate(parts[0]);
		int[] right = evaluate(parts[1]);
		
		if (left[0] == right[0]) {
			if (left[1] == right[1]) return INFINITE_SOLUTION;
	...
	private int[] evaluate(String s) {
		int[] result = new int[2];  // first element is the coefficient, second is the constant
		
		// regex, https://stackoverflow.com/questions/10804732/what-is-the-difference-between-and-in-regex
		// x+5-2x => {x, +5, -2x}
		String[] tokens = s.split("(?=[+-])"); // ()for match group; ?= for match and include in res; [+-] means + or -;
		for (String token : tokens) {
			if (token.equals("x") || token.equals("+x")) result[0]++;
			else if (token.equals("-x")) result[0]--;
			else if (token.contains("x")) {
				result[0] += Integer.parseInt(token.substring(0, token.length()-1)); // need to include the sign here
			}
			else {
