...
public class FlipGame {
     ...
     * valid move. For example, given s = "++++", after one move, it may become
     * one of the following states: [ "--++", "+--+", "++--" ] If there is no
     * valid move, return an empty list [].
     * 
    ...
    public List<String> generatePossibleNextMoves(String s) {
        ...
        /*
         * int i = 0; while (i < s.length() - 1) { if (s.charAt(i) == '+') { if
         * (s.charAt(i+1) == '+') { result.add(s.substring(0, i) + "--" +
         * s.substring(i+2)); i++; // only increment by one when there two
         * consecutive + } else i += 2; // skip - and go to the next char } else
         * { i++; } }
         */
        return result;
    }
    ...
    public List<String> generatePossibleNextMoves_withIndexOf(String s) {
        ...
        for (int i = -1; (i = s.indexOf("++", i + 1)) >= 0;) {
            result.add(s.substring(0, i) + "--" + s.substring(i + 2)); // won't
                                                                       // throw
    ...
    // see if the opponent can win or not,
    // if the opponent cannot win, great, we win!
    // There are at most n - 1 ways to replace "++" to "--" (imagine s is all
    // "+++..."), once we replace one "++",
    // there are at most (n - 2) - 1 ways to do the replacement, the time
    // complexity is (n - 1) x (n - 3) x (n - 5) x ..., so it's O(n!!), double
    // factorial.
    public boolean canWin(String s) {
        if (s == null || s.length() < 2)
            return false;
        /*
         * for (int i = 0; i < s.length()-1; i++) { if (s.startsWith("++", i)) {
         * String t = s.substring(0, i) + "--" + s.substring(i+2); if
         * (!canWin(t)) return true; }
         * 
         * } return false;
         */
