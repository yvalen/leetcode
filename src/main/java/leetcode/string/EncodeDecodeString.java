...
public class EncodeDecodeString {
	...
	// Use escape
	// Double any hashes inside the strings, then use stand alone hashes (surrounded by spaces) to mark 
	...
	public String encode_withEscape(List<String> strs) {
		...
		for (String str : strs) {
			// need to append the result of replace call, cannot append str because str is immutable
	...
	public List<String> decode_withEscape(String s) {
		...
		// the array returned by split contains each substring of this string that is terminated by another substring 
		// that matches the given expression or is terminated by the end of the string
		// If limit is non-positive then the pattern will be applied as many times as possible and the array can have 
		// any length. If limit is zero then the pattern will be applied as many times as possible, the array can have
	...
	public List<String> decode(String s) {
		...
		while (start < s.length()) {
			// Any '/' in original strs do not conflict since i is moved forward to pick up next size value.
