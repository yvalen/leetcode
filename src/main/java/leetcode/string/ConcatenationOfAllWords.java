...
public class ConcatenationOfAllWords {
	...
	/*
	 * Say in words there are m strings with length n. What is required to match in S is a length of 
	 * m*n string start with each position in S. And in the m*n long string, every string in L appear only once.
	 * Algorithm is: scan every m*n long string start from each position in S, see if all the strings in words 
	...
	public List<Integer> findSubstring_bruteforce(String s, String[] words) {
		...
		if (s == null || s.isEmpty() || words == null || 
				words.length == 0 || s.length() < words[0].length()) {
			return result;
		}
		...
		for (int i = 0; i <= s.length() - words.length * len; i++) {
			...
			for (int j = 0; j < words.length; j++ ) {
				...
				if (currCount.get(word) != null) {
					...
					if (currCount.isEmpty()) result.add(i);
				}
				else {
