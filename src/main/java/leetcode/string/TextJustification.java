...
public class TextJustification {
	public List<String> fullJustify(String[] words, int maxWidth) {
        ...
        for (int start = 0; start < words.length; start = end) {
        	...
        	int space = 1; // there is at least one space between words, accounted for in the previous loop
        	int extra = 0; // extra space to fill 
        	int numOfGaps = end - start - 1; // n words has n-1 spaces between them
        	
        	// don't need to calculate this if only one word can be accommodated in the line
        	// or the last word is reached
        	if (end != start + 1 && end != words.length) {
        		space = (maxWidth - len) / numOfGaps + 1; // plus 1 accounts for the space added in the previous loop
        		extra = (maxWidth - len) % numOfGaps;
        	}
        	
        	StringBuilder sb = new StringBuilder(words[start]); // initialize with the first word
