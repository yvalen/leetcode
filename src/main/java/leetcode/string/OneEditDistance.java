		...
		for (int i = 0; i < Math.min(s.length(),  t.length()); i++) {
			if (s.charAt(i) != t.charAt(i)) { // the first different character
				if (s.length() == t.length()) { 
					// s and t have the same length, we can only replace the first different char
					return s.substring(i+1).equals(t.substring(i+1));
				}
				
				if (s.length() < t.length()) {
					// s is shorter than t, we can only delete the first different char from t
					return s.substring(i).equals(t.substring(i+1));
				}
				
				// s is longer than t, we can only delete the first different cgar from s
				return s.substring(i+1).equals(t.substring(i));
			}
		}
		
		// all characters are same so far, we can only delete the last character of the longer one
