...
public class LongestPalindrome {
	...
	public String longestPalindrome(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
        	...
        	int len2 = getLengthOfPalindrome(s, i, i+1); // handle center between two letters, e.g. abba
	...
	private int getLengthOfPalindrome(String s, int left, int right) {
		...
		return (right - left - 1); // should minus 1 since left and right are beyong boundry
	...
	public String longestPalindrome_dp(String s) {
		...
		// dp[i][j] indicates whether substring starting at i and ending at j is palindrome
	 ...
	 * A longest palin­dromic sub­se­quence is a sequence that appears in the same rel­a­tive order, but not nec­es­sar­ily contiguous(not sub­string) 
	 * and palin­drome in nature( means the sub­se­quence will read same from the front and back.
	...
	public int longestPalindromeSubseq(String s) {
		...
		int n = s.length();
		// dp[i][j] stores the length of the longest palindrome subsequence in s(i, j)
        int[][] dp = new int[n][n];
        for (int i = n - 1; i >= 0; i--) {
        	dp[i][i] = 1;
        	for (int j = i + 1; j < n; j++) {
