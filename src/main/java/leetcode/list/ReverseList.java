...
public class ReverseList {
	 ...
	 * 
	 * Company: Facebook, Microsoft, Bloomberg, Amazon, Uber, Twitter, Apple, Snapchat, Zenefits, Yelp, Yahoo, Adobe
	 * Difficulty: easy
	 * Similar Questions: 92(Reverse Linked List II)
	 */
	...
	public ListNode reverseListRecursive(ListNode head) {
		...
		head.next.next = head;
        head.next = null;  // need to set head.next to null here, otherwise will get into infinite loop
	...
	public ListNode reverseBetween(ListNode head, int m, int n) {
		...
		for (int i = m; i < n; i++) {  // need to check for < n here
			...
			next.next = prev.next; // bubble the element to the beginning of the reversed list
			prev.next = next;
		}
	...
	public ListNode reverseBetween_twoPass(ListNode head, int m, int n) {
        ...
        if (before != null) before.next = null; // need to check for null here as we could reverse from the beginning
