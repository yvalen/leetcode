...
public class MergeList {
	 ...
	 * Difficulty: easy
	 * Similar Questions: 244(WordDistiance)
	 */
	...
	public ListNode mergeKLists_withMinHeap(ListNode[] lists) {
		...
		for (ListNode list : lists) {
			if (list != null) { // need to check for null here because the list element could be null, e.g. [[]]
				minHeap.add(list);
			}
		}
	...
	// 3. Split the list into two halves, recursively merge the lower half and the upper half, 
	...
	private ListNode mergeLists(ListNode[] lists, int lo, int hi) {
		...
		if (lo == hi) return lists[lo]; // single list
		if (lo == hi - 1) return mergeTwoLists(lists[lo], lists[hi]); // two lists
